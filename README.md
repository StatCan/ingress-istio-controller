...
Il été testé avec la version 1.5 d'Istio et les versions 1.17 et 1.18 de Kubernetes. Ceci dit, il devrait être compatible avec toutes versions d'Istio.

L'annotation `kubernetes.io/ingress.class` ainsi que l'object IngressClass peuvent être utilisé afin de cibler les Ingresses devrant être gérer par le contrôleur.


#### Annotation `kubernetes.io/ingress.class`

Débutant en Kubernetes 1.18, `kubernetes.io/ingress.class` [est dépriciée *(anglais)*](https://kubernetes.io/docs/concepts/services-networking/ingress/#deprecated-annotation)
en faveur d'utilisation de l'IngressClass. L'annotation peut encore être utilisée comme cible par ce contrôleur et comme documenté sur le
...
Suivant est un example d'un IngressClass pouvant être utiliser:
...
Il y a deux façon d'altérer le fonctionnemt du **isto-ingress-controller**. 
...
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| --kubeconfig             | Défini le chemin de fichier local au kubeconfig. <br>*Seulement requis à l'extérieur du cluster.*                                                                                                                                                                | ""                                           |
...
| --default-gateway        | Le nom du Istio Gateway duquel les VirtualServices seront servis. <br>L'argument devrait être en format **\<namespace>/\<nom>**.                                                                                                                                 | istio-system/istio-autogenerated-k8s-ingress |
...
Des Annotations peuvent être ajouter aux Ingresses afin de modifier le fonctionnement du contrôleur. Ci-dessous est une tables des annotations possible :

| Annotation                     | Description                                                                                                  | Value Type             | Example Values               |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------ | ---------------------- | ---------------------------- |
| ingress.statcan.gc.ca/ignore   | Cause que le contrôleur ne cible pas l'Ingress annoté.                                                       | booléen                | "true"                       |
| ingress.statcan.gc.ca/gateways | Une liste de nom de Gateway séparé par virgules devrant être référée par le VirtualService au lieu du default-gateway. | string séparé par virgules | mesh,production/prod-gateway |
